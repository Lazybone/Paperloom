...
static void store_cache_entry(const String& key, const ThumbContext& ctx) {
    ...
    if (!entry.pixels) {
        // Without this log, repeated decode-on-every-render after a PSRAM
        // exhaustion produced no diagnosable trail.
        Serial.printf("Cover cache: ps_malloc(%u) failed for %s — re-decode each render\n",
                      (unsigned)bytes, key.c_str());
        return;
    }
