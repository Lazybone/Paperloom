...
// Library uses its own header — adds the central "Sort: …" indicator that
...
static void drawPosterNoCoverTile(const BookInfo& book, int x, int y, int w, int h) {
    ...
    display_draw_text(x + (w - infoW) / 2, y + h - 38, info.c_str(), 0);
    ...
    if (book.author.length() > 0) {
        ...
        display_draw_text(x + (w - authorW) / 2, y + h - 68, author.c_str(), 0);
    }
    ...
    if (book.hasProgress && book.totalChapters > 0) {
        ...
        display_draw_text(x + 14, y + h - 18, pctStr, 0);
    }
    // Author was already drawn above at y+h-68; the duplicate render at
    // y+h-48 produced an overlapping ghost. Removed.
}
...
static void drawDefaultPoster(BookInfo& book, int x, int y, int w, int h) {
    ...
    const char* band = "BOOK";
    int bandW = display_text_width(band);
    display_draw_text(x + (w - bandW) / 2, y + 32, band, 0);
...
void ui_library_draw(
    ...
    if (books.empty()) {
        ...
        if (numVisible > cardsPerPage) {
            ...
            if (hasPrev) display_draw_text(MARGIN_X, pageInfoY, "< Prev", 0);
            if (hasNext) {
                const char* nextLbl = "Next >";
                int nw = display_text_width(nextLbl);
                display_draw_text(W - MARGIN_X - nw, pageInfoY, nextLbl, 0);
            }
        ...
        for (int vi = scroll; vi < numVisible && vi < scroll + maxVis; vi++) {
            ...
            if (pct >= 0) {
                ...
                display_draw_text(rowX + rowW - pw - 10, itemY + titleBaselineY, pctStr, 0);
            }
            ...
            if (info.length() > 0) display_draw_text(rowX + 10, itemY + metaBaselineY, info.c_str(), 0);
        }
        ...
        if (numVisible > maxVis) {
            ...
            if (hasPrev) display_draw_text(MARGIN_X, pageControlY, "< Prev", 0);
            if (hasNext) {
                const char* nextLbl = "Next >";
                int nw = display_text_width(nextLbl);
                display_draw_text(W - MARGIN_X - nw, pageControlY, nextLbl, 0);
            }
            ...
            snprintf(pageStr, sizeof(pageStr), "%d / %d", curPage, totalPages);
            int pw = display_text_width(pageStr);
            display_draw_text((W - pw) / 2, pageControlY, pageStr, 0);
        }
    }
...
AppState ui_library_touch(
    ...
    if (y < HEADER_HEIGHT) {
        ...
        scroll = 0;
        if (!settings_save()) Serial.println("Settings: save failed (sort cycle)");
