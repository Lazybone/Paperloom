...
static void configureSecureClient(WiFiClientSecure& c) {
    // Pin the project-wide trust bundle. The Arduino-ESP32 default build
    // does NOT embed a Mozilla bundle, so without this we'd fall back to
    // setInsecure() — MITM-vulnerable. The bundle covers api.github.com
    // (USERTrust ECC), the release CDN (ISRG Root X1), and DigiCert as
    // a historical fallback. See include/tls_certs.h — update
    // PAPERLOOM_TRUSTED_ROOTS there when issuing CAs rotate anchors.
    c.setCACert(PAPERLOOM_TRUSTED_ROOTS);
